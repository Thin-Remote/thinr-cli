...
export function productCommand(program) {
    ...
    const registerProductFsCommand = ({ name, description, args, build, perDeviceLabel }) => {
        ...
        cmd.action(async (...commandArgs) => {
            ...
            const updateSpinnerText = () => {
                runSpinner.text =
                    `${perDeviceLabel(rest)} on ${devices.length} device(s) — ` +
                    `${done}/${devices.length} done · ${inFlight} in flight` +
                    (opts.failFast ? ' · fail-fast' : '');
            };
            updateSpinnerText();
    ...
    product
        ...
        .action(async (productId, localPath, remotePath, opts, cmd) => {
            ...
            const updateSpinnerText = () => {
                runSpinner.text =
                    `Uploading ${localPath} → ${remotePath} on ${devices.length} device(s) — ` +
                    `${done}/${devices.length} done · ${inFlight} in flight` +
                    (opts.failFast ? ' · fail-fast' : '');
            };
            updateSpinnerText();
