...
import { requireConfig, resolveUser } from './config.js';
import { BYTES, TIMEOUTS } from './constants.js';
import { CommandStream } from './command-stream.js';
import { ensureFreshToken, refreshToken } from './auth.js';
...
export function createDeviceAPI(deviceId, options = {}) {
    ...
    const apiUser = resolveUser(options.user);
    const base = device(apiUser, deviceId);
    const resourcePath = `${base}/resources`;
    const filesPath = `${base}/files`;
    ...
    return {
         ...
         *
         * Auth: the WS handshake has no refresh-on-401 interceptor like
         * HTTP calls do, so the token is refreshed up front when its JWT
         * is already expired, and a handshake 401 gets one explicit
         * refresh-and-retry.
         */
        async execStream(cmd, { timeout = 0, onStdout, onStderr, onCancel, stdin } = {}) {
            ...
            try {
                return await run((await ensureFreshToken()) || config.token);
            } catch (error) {
                // A token that decodes as still-valid can be rejected
                // anyway (revoked, clock skew): mirror the api.js
                // interceptor and retry the handshake once after an
                // explicit refresh.
