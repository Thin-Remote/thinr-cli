...
export async function startMCPServer() {
    ...
    // Default device/user from environment (set by thinr env)
    const defaultDevice = process.env.THINR_DEVICE || null;
    const defaultUser = process.env.THINR_USER || null;

    /** Helper to get API for a device */
    function getAPI(device, user) {
        const d = device || defaultDevice;
        const u = user || defaultUser || undefined;
        if (!d) throw new Error('No device specified. Use thinr_devices to list available devices.');
        return { api: createDeviceAPI(d, { user: u }), device: d };
    }
    ...
    const tools = [
        ...
        },
        {
            ...
            inputSchema: {
                ...
                properties: {
                    device: {
                        type: 'string',
                        description: `Device ID${defaultDevice ? ` (default: ${defaultDevice})` : ''}`
                    },
                    user: {
                        type: 'string',
                        description: `API user${defaultUser ? ` (default: ${defaultUser})` : ''}`
                    }
                },
                required: []
            }
        ...
        {
            name: 'thinr_exec',
            description: `Execute a shell command on a remote device. Returns stdout, stderr, and exit code.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    command: {
                    ...
                    device: {
                        type: 'string',
                        description: `Device ID${defaultDevice ? ` (default: ${defaultDevice})` : ' (from thinr_devices)'}`
                    },
                    timeout: {
                        type: 'number',
                    ...
                    user: {
                        type: 'string',
                        description: `API user${defaultUser ? ` (default: ${defaultUser})` : ' (for admin impersonation)'}`
                    }
                },
                required: ['command']
            }
        ...
        {
            name: 'thinr_read',
            description: `Read a file from a remote device. Paths are absolute on the device filesystem (e.g., "/etc/hosts", "/var/log/syslog"). Subject to the OS user's permissions; the agent currently runs as root on most devices.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    path: {
        ...
        {
            name: 'thinr_write',
            description: `Write content to a file on a remote device. Paths are absolute on the device filesystem (e.g., "/etc/hosts", "/var/log/syslog"). Subject to the OS user's permissions; the agent currently runs as root on most devices.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    path: {
        ...
        {
            name: 'thinr_ls',
            description: `List directory contents on a remote device. Paths are absolute on the device filesystem (e.g., "/", "/etc", "/var/log"). Subject to the OS user's permissions; the agent currently runs as root on most devices. Use include_hidden to see dotfiles.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    path: {
        ...
        {
            name: 'thinr_mkdir',
            description: `Create a directory on a remote device. Paths are absolute on the device filesystem (e.g., "/etc/hosts", "/var/log/syslog"). Subject to the OS user's permissions; the agent currently runs as root on most devices.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    path: {
        ...
        {
            name: 'thinr_delete',
            description: `Delete a file or directory on a remote device. Paths are absolute on the device filesystem (e.g., "/etc/hosts", "/var/log/syslog"). Subject to the OS user's permissions; the agent currently runs as root on most devices.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    path: {
        ...
        {
            name: 'thinr_move',
            description: `Move or rename a file or directory on a remote device. Paths are absolute on the device filesystem (e.g., "/etc/hosts", "/var/log/syslog"). Subject to the OS user's permissions; the agent currently runs as root on most devices.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    source: {
        ...
        },
        {
            ...
            description: `Invoke a resource on a device. Pass \`inputs\` when the resource declares an input schema (e.g. cmd, update); omit for read-only resources like monitoring/system/agent. Returns the resource's output. Use thinr_resource_list first to discover available resources and their expected inputs.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    resource: {
        ...
        },
        {
            ...
            description: `List the custom scripts registered by the agent's scripts extension, including the on-device scripts directory and each script's --describe schema. Scripts are user-provided executables in <agent_base>/scripts/ that the agent registers as first-class resources under their own names (e.g. a file named \`battery.sh\` becomes the resource \`battery\`). Use this to discover which scripted capabilities the device exposes.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    device: {
...
This tool writes the script, makes it executable, and calls the agent's \`$scripts/reload\` so the change takes effect without restarting the agent.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    name: {
        ...
        },
        {
            ...
            description: `Remove a previously installed custom script from the device and reload the scripts extension so the corresponding resource is unregistered. Use this to retire a scripted capability you no longer want the device to expose.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    name: {
        ...
        {
            name: 'thinr_update',
            description: `Check for agent updates or apply an update on a remote device.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    device: {
        ...
        },
        {
            ...
            description: `Get monitoring data (CPU, memory, disk, network, temperature, load) from a device or all devices. Use realtime=true for live data from the device, or omit for historical data from the monitoring bucket.${defaultDevice ? ` Default device: ${defaultDevice}` : ''}`,
            inputSchema: {
                type: 'object',
                properties: {
                    realtime: {
                    ...
                    device: {
                        type: 'string',
                        description: `Device ID. Required for realtime. Omit for historical data from ALL devices.${defaultDevice ? ` (default: ${defaultDevice})` : ''}`
                    },
                    items: {
                        type: 'number',
...
The file extension (.sh, .py, .js, …) is preserved in the storage for editor highlighting, but the API resource name is the stem without extension. So scripts/backup.sh is callable as the resource "backup".`,
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            switch (name) {
                case 'thinr_devices': {
                    const devices = await getDevices({}, args.user || null);
                ...
                case 'thinr_search': {
                    ...
                    const matches = await getDevices({ name: query }, args.user || null);
                ...
                case 'thinr_resource_list': {
                    const device = args.device || defaultDevice;
                ...
                case 'thinr_script_write': {
                    ...
                    if (!/^[A-Za-z0-9._-]+$/.test(args.name)) {
                        throw new Error(`Invalid script name "${args.name}" (allowed: letters, digits, dot, dash, underscore)`);
                    }
                ...
                case 'thinr_script_delete': {
                    ...
                    if (!/^[A-Za-z0-9._-]+$/.test(args.name)) {
                        throw new Error(`Invalid script name "${args.name}"`);
                    }
                    const scriptPath = `${baseDir}/${args.name}`;
                ...
                case 'thinr_monitoring': {
                    const u = args.user || defaultUser || undefined;
                    const apiUser = u || readConfig().username;
                    const device = args.device || defaultDevice;

                    // Real-time: call the device resource directly
                    if (args.realtime) {
                        if (!device) throw new Error('Device required for real-time monitoring');
                        const { api: deviceApi } = getAPI(device, args.user);
                        const mon = await deviceApi.getResource('monitoring');
                    ...
                    if (args.minutes) {
                        params.min_ts = Date.now() - (args.minutes * 60 * 1000);
                        params.max_ts = 0;
                    } else {
                        if (args.min_ts) params.min_ts = args.min_ts;
                        if (args.max_ts !== undefined) params.max_ts = args.max_ts;
                    }
                    if (args.agg) params.agg = args.agg;
                    if (args.agg_type) params.agg_type = args.agg_type;
                ...
                case 'thinr_product_delete': {
                    if (!args.product) throw new Error('product is required');
                    const apiUser = args.user || defaultUser || readConfig().username;
                    const steps = [];

                    try {
                        await api.delete(`/v1/users/${apiUser}/products/${args.product}`);
                        steps.push(`deleted product "${args.product}"`);
                    } catch (e) {
                        if (e.response?.status !== 404) throw e;
                        steps.push(`product "${args.product}" already absent`);
                    }

                    if (!args.keep_storage) {
                        try {
                            await api.delete(`/v1/users/${apiUser}/storages/${args.product}`);
                            steps.push(`deleted file storage "${args.product}"`);
                        } catch (e) {
                            if (e.response?.status !== 404) throw e;
                            steps.push(`file storage "${args.product}" already absent`);
                        }
                    } else {
                        steps.push(`kept file storage "${args.product}" (keep_storage=true)`);
                    }
                ...
                case 'thinr_products': {
                    const apiUser = args.user || defaultUser || readConfig().username;
                    const res = await api.get(`/v1/users/${apiUser}/products`);
                    ...
                    const lines = await Promise.all(products.map(async p => {
                        ...
                        let scripts = 0;
                        try {
                            const api_ = (await api.get(`/v1/users/${apiUser}/products/${id}/profile/api`)).data || {};
                            scripts = Object.keys(api_).length;
                        } catch (e) {}
                ...
                case 'thinr_device_set_product': {
                    ...
                    if (args.product === undefined || args.product === null) throw new Error('product is required (pass "" to unassign)');
                    const apiUser = args.user || defaultUser || readConfig().username;
                    await api.put(`/v1/users/${apiUser}/devices/${device}`, { product: args.product });
                ...
                case 'thinr_product_script_list': {
                    if (!args.product) throw new Error('product is required');
                    const apiUser = args.user || defaultUser || readConfig().username;
                    const storage = args.product;

                    let files = [];
                    try {
                        const res = await api.get(`/v1/users/${apiUser}/storages/${storage}/files`);
                        files = (res.data || []).filter(f => f.type === 'file' && f.path.startsWith('scripts/'));
                    } catch (e) {
                        if (e.response?.status !== 404) throw e;
                    }
                    ...
                    let apis = {};
                    try {
                        const res = await api.get(`/v1/users/${apiUser}/products/${args.product}/profile/api`);
                        apis = res.data || {};
                    } catch (e) {
                        if (e.response?.status !== 404) throw e;
                    }
                    ...
                    const header = files.length === 0
                        ...
                        : `${files.length} script(s) in product "${args.product}":`;
                    const lines = files.map(f => {
                        const stem = f.name.replace(/\.[^.]+$/, '');
                        const marker = apis[stem] ? '[api]' : '[orphan]';
                        return `  ${marker} ${f.name}  (${f.size} bytes) → resource: ${stem}`;
                    });
                    return {
                        content: [{ type: 'text', text: [header, ...lines].join('\n') }],
                        isError: false
                ...
                case 'thinr_product_script_read': {
                    ...
                    const res = await api.get(
                        `/v2/users/${apiUser}/storages/${args.product}/files/scripts/${args.name}`,
                ...
                case 'thinr_product_script_write': {
                    ...
                    // 1. Ensure product exists — create it with the ThinRemote scaffold if missing.
                    // Mirrors the product payload the ThinRemote agent provisions: type=thinremote,
                    // a Font Awesome icon (defaults to Linux), and the monitoring bucket.
                    try {
                        await api.get(`/v1/users/${apiUser}/products/${args.product}`);
                    } catch (e) {
                        if (e.response?.status !== 404) throw e;
                        const icon = args.icon || 'fab fa-linux';
                        await api.post(`/v1/users/${apiUser}/products`, {
                            product: args.product,
                            name: args.product,
                            description: 'ThinRemote product',
                            enabled: true,
                            config: {
                                type: 'thinremote',
                                icons: [{
                    ...
                    if (!indexExists) {
                        await api.put(
                            `/v2/users/${apiUser}/storages/${storage}/files/index.js?overwrite=true`,
                            indexJs,
                    ...
                    // 3. Upload the script body
                    await api.put(
                        `/v2/users/${apiUser}/storages/${storage}/files/scripts/${args.name}?overwrite=true`,
                        args.content,
                        { headers: { 'Content-Type': 'text/plain' } }
                    );
                    steps.push(`wrote scripts/${args.name} (${args.content.length} bytes)`);

                    // 4. Ensure profile.code.storage points at this storage
                    const code = (await api.get(`/v1/users/${apiUser}/products/${args.product}/profile/code`)).data || {};
                    if (code.storage !== storage) {
                        await api.put(`/v1/users/${apiUser}/products/${args.product}/profile/code`, {
                            storage, code: '', version: '1.0'
                        });
                        steps.push(`linked profile.code.storage → ${storage}`);
                    }

                    // 5. Ensure product is enabled
                    const prod = (await api.get(`/v1/users/${apiUser}/products/${args.product}`)).data || {};
                    if (!prod.enabled) {
                        await api.put(`/v1/users/${apiUser}/products/${args.product}`, { enabled: true });
                        steps.push('enabled product');
                    }

                    // 6. Create/update the API resource
                    await api.put(
                        `/v1/users/${apiUser}/products/${args.product}/profile/api/${stem}`,
                        {
                            enabled: true,
                ...
                case 'thinr_product_script_delete': {
                    ...
                    try {
                        await api.delete(`/v2/users/${apiUser}/storages/${args.product}/files/scripts/${args.name}`);
                        steps.push(`deleted scripts/${args.name}`);
                    } catch (e) {
                        if (e.response?.status !== 404) throw e;
                        steps.push(`scripts/${args.name} already absent`);
                    }

                    try {
                        await api.delete(`/v1/users/${apiUser}/products/${args.product}/profile/api/${stem}`);
