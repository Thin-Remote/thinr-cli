...
import { getDevices, getDevice } from './device.js';
...
export async function startMCPServer() {
    ...
    const tools = [
        {
            name: 'thinr_devices',
            description: 'List remote devices with their online/offline status. Pass `query` to filter by device id or name (case-insensitive regex on the server side, so plain substrings, anchors, wildcards and alternations all work). Use this before any other device tool to discover or narrow down ids.',
            inputSchema: {
                type: 'object',
                properties: {
                    query: {
                        type: 'string',
                        description: 'Optional pattern matched against device id and name. Plain substrings work ("office"); regex metacharacters are also honoured ("^raspberry", "mac|book"). Case-insensitive.'
                    },
                    limit: {
                        type: 'number',
                        description: 'Maximum matches to return when `query` is set (default: 20). Ignored when listing without a query.'
                    },
                    user: {
                        type: 'string',
        ...
        try {
            ...
            switch (name) {
                case 'thinr_devices': {
                    ...
                    if (!query) {
                        return {
                            content: [{ type: 'text', text: devices.map(renderLine).join('\n') || 'No devices found' }],
                            isError: false,
                        };
                    }
                ...
                case 'thinr_device_info': {
                    ...
                    try {
                        ...
                        if (record.connection) {
                            ...
                            meta.push(`connection:    ${c.active ? 'online' : 'offline'}${c.ip_address ? ` from ${c.ip_address}` : ''}`);
                        }
                        if (meta.length) {
                    ...
                    // Try system_info resource next (structured data from the agent)
                    try {
                ...
                case 'thinr_exec': {
                    ...
                    // Default to 30s so a hung command doesn't lock the
                    // session indefinitely. Callers can pass a larger value
                    // for known-long operations (compiles, backups, …).
                    const timeout = Number.isFinite(args.timeout) && args.timeout > 0 ? args.timeout : 30;
                    let output = '';
                ...
                case 'thinr_resource_list': {
                    ...
                    if (!device) throw inputError('No device specified. Use thinr_devices to list available devices.');
