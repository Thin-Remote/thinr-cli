...
export async function startMCPServer() {
    ...
    // Default device/user from environment, set by `thinr mcp -d <id>` or
    // by the host that spawns the server (e.g. `claude mcp add … -- thinr mcp -d <id>`).
        ...
        try {
            ...
            switch (name) {
                ...
                case 'thinr_device_set_product': {
                    ...
                    await setDeviceProduct(device, args.product, args.user || defaultUser);
                ...
                case 'thinr_product_script_list': {
                    if (!args.product) throw new Error('product is required');
                    const entries = await listProductScripts(args.product, args.user || defaultUser);
                ...
                case 'thinr_product_script_read': {
                    if (!args.product) throw new Error('product is required');
                    if (!args.name) throw new Error('name is required');
                    const body = await readProductScript(args.product, args.name, args.user || defaultUser);
                    return {
