...
import { formatUptime, colorPct, formatBytes, label, muted, success, error } from './format.js';
 ...
 */
export async function getDeviceStatus(deviceId, user) {
    try {
        const response = await api.get(`${v1(user)}/devices/${deviceId}/stats`);
...
export function formatDeviceStatus(deviceId, status, monitoring = null) {
    const connectedStatus = status.connected ? success('● Online') : error('○ Offline');
    ...
    const lines = [''];
    lines.push(label(`Device: ${deviceId}`));
    ...
    if (monitoring) {
        ...
        if (monitoring.cpu?.usage != null) {
            ...
            const detail = bits.length ? muted(` (${bits.join(', ')})`) : '';
        ...
        if (monitoring.memory?.usage != null) {
            ...
            const detail =
                used != null && total != null
                    ? muted(` (${formatBytes(used)} of ${formatBytes(total)} used)`)
                    : '';
        ...
        if (monitoring.disk?.root?.usage != null) {
            ...
            const detail =
                root.available != null && root.total != null
                    ? muted(` (${formatBytes(root.available)} free of ${formatBytes(root.total)})`)
                    : '';
            lines.push(`  Disk:    ${colorPct(root.usage)}${detail}`);
        }
        if (monitoring.load) {
            const l = monitoring.load;
            const parts = [l['1m'], l['5m'], l['15m']].map((v) => (v != null ? v.toFixed(2) : '—'));
        ...
        if (monitoring.agent?.version) {
            lines.push(`  Agent:   ${muted(monitoring.agent.version)}`);
        }
    }

    lines.push('');
    lines.push(label('Data Transfer:'));
