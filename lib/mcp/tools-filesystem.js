...
async function toolLs(args) {
    ...
    // "/" is the agent's base directory, not the filesystem root, while every
    // other absolute path is literal. Listing "/" on an agent based in
    // /var/log and reading it as the root is an easy and wrong conclusion to
    // draw, so say which directory these entries actually came from.
    let header = '';
    if (requested === '/') {
        try {
            const root = await api.info('/');
            if (root?.path && root.path !== '/') {
                header = `listing ${root.path} (the agent's base directory, which "/" resolves to)\n`;
            }
        } catch {
...
export const tools = [
    ...
    {
        name: 'thinr_ls',
        description: `List directory contents on a remote device. Paths are absolute on the device filesystem (e.g., "/etc", "/var/log"), with one exception: "/" on its own resolves to the agent's base directory (\`fs_base_path\` in thinr_device_info), which for a system install is usually /var/log. The listing says which directory it came from when the two differ. Nothing is confined to that directory — "/etc" reads the real /etc — so do not read a "/" listing as the root filesystem. Subject to the OS user the agent runs as; check whoami (or thinr_device_info) before assuming write access to system paths. Use include_hidden to see dotfiles.`,
        inputSchema: {
            type: 'object',
            properties: {
                path: {
                    type: 'string',
                    description:
                        'Directory path. Absolute and literal, except "/", which means the agent\'s base directory (default: "/").',
                },
                include_hidden: {
                    type: 'boolean',
