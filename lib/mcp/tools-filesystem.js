...
async function toolRead(args) {
    const { api } = getAPI(args.device_id, args.user);
...
async function toolLs(args) {
    ...
    const entries = await api.listDir(requested, args.include_hidden || false);

    // "/" resolves through the agent process's working directory, normally
    // the filesystem root, while every other absolute path is literal. An
    // agent started elsewhere answers "/" with that directory instead, which
    // reads as the device's root and is not, so name the directory these
    // entries actually came from.
    let header = '';
    if (requested === '/') {
        try {
            const root = await api.info('/');
            if (root?.path && root.path !== '/') {
                header = `listing ${root.path} (the agent's working directory, which "/" resolves to)\n`;
            }
        } catch {
            // Labelling is a convenience; a listing without it still stands.
        }
    }
...
export const tools = [
    ...
    },
    {
        ...
        inputSchema: {
            ...
            required: ['path', 'content', 'device_id'],
        },
        handler: toolWrite,
    },
    {
        name: 'thinr_ls',
        description: `List directory contents on a remote device. Paths are absolute on the device filesystem (e.g., "/etc", "/var/log"), with one exception: "/" on its own resolves to the agent process's working directory, normally the filesystem root, reported as \`fs_base_path\` by thinr_device_info. When it is not the root — an agent started from elsewhere — the listing says which directory it came from. Nothing is confined to it: "/etc" reads the real /etc. Subject to the OS user the agent runs as; check whoami (or thinr_device_info) before assuming write access to system paths. Use include_hidden to see dotfiles.`,
        inputSchema: {
            type: 'object',
            properties: {
                path: {
                    type: 'string',
                    description:
                        'Directory path. Absolute and literal, except "/", which means the agent\'s working directory, normally the filesystem root (default: "/").',
                },
                include_hidden: {
                    type: 'boolean',
    ...
    },
    {
        ...
        inputSchema: {
            ...
            required: ['source', 'destination', 'device_id'],
        },
        handler: toolMv,
    },
    {
