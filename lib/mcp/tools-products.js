...
async function toolProductWrite(args) {
    ...
    const concurrency =
        Number.isFinite(args.concurrency) && args.concurrency > 0
            ...
            : 10;
    const failFast = !!args.fail_fast;
    const includeOffline = !!args.all;

    const filter = { productId: args.product };
    if (args.group) filter.asset_group = args.group;
    let devices = await getDevices(filter, args.user);
    ...
    const poolResults = await runPool(devices, concurrency, async (device) => {
        if (failFast && aborted) {
            return {
                ...
                bytes: 0,
                durationMs: 0,
                error: `skipped (fail-fast after ${firstFailureDevice})`,
            };
        }
        const t0 = Date.now();
        try {
            ...
            if (failFast && !aborted) {
                aborted = true;
                firstFailureDevice = device.device;
            }
            return {
                device: device.device,
                ok: false,
...
async function toolProductExec(args) {
    ...
    const concurrency =
        Number.isFinite(args.concurrency) && args.concurrency > 0
            ...
            : 10;
    const timeout =
        Number.isFinite(args.timeout) && args.timeout > 0 ? Math.floor(args.timeout) : 30;
    const failFast = !!args.fail_fast;
    const includeOffline = !!args.all;
    ...
    // Worker never throws: it returns a populated `ExecEntry` in every
    // branch (success, non-zero exit, timeout, connection error, or
    // "skipped" once fail-fast has triggered). That keeps the post-pool
    // mapping trivial and sidesteps the `{ ok: true } | { ok: false }`
    // union narrowing limitation with JSDoc types.
    const poolResults = await runPool(devices, concurrency, async (device) => {
        if (failFast && aborted) {
            /** @type {ExecEntry} */
            const skipped = {
        ...
        if (failFast && !entry.ok && !aborted) {
            aborted = true;
            firstFailureDevice = entry.device;
        }
        return entry;
    });
    ...
    const entries = poolResults.map((r, i) => {
        if (r && r.ok) return r.value;
        // Defensive fallback — the worker above never throws, so this
        // branch shouldn't trigger in practice.
        return {
            device: devices[i].device,
            ok: false,
            exitCode: null,
    ...
    lines.push(
        `\nSummary: ${okCount} ok, ${failCount} failed, ${timedOutCount} timed out, ${errorCount} errored — ${totalMs}ms total`,
    );
