...
async function toolProductCreate(args) {
    if (!args.product) throw inputError('product is required');
    if (!/^[A-Za-z0-9._-]+$/.test(args.product)) {
        throw inputError(
            `Invalid product id "${args.product}" (allowed: letters, digits, dot, dash, underscore)`,
        );
    }
    if (await productExists(args.product, args.user)) {
        const err = new Error(`Product "${args.product}" already exists`);
    ...
    let iconSpec;
    if (args.icon) {
        if (!args.icon.source) throw inputError('icon.source is required when icon is set');
        const type = args.icon.type || 'fa';
        if (type !== 'fa' && type !== 'svg') {
            throw inputError(`icon.type must be "fa" or "svg" (got "${type}")`);
        }
        iconSpec = {
            type,
            source: args.icon.source,
            color: args.icon.color || '#000000',
            background: args.icon.background || '#ffffff',
        };
    }

    await createProduct(args.product, args.user, {
        name: args.name,
        description: args.description,
        ...(iconSpec ? { icon: iconSpec } : {}),
    });
    ...
    return {
        content: [
            {
                type: 'text',
                text: `Created product "${args.product}"${args.name ? ` (${args.name})` : ''}.`,
            },
        ],
        isError: false,
    };
...
async function toolProductUpdate(args) {
    if (!args.product) throw inputError('product is required');
    ...
    if (args.icon) {
        ...
        // Fetch current config so the icon update doesn't clobber unrelated
        // fields (`type`, custom dashboards, profile-side bits the API
        // groups under config).
        const current = await getProduct(args.product, args.user);
    ...
    if (fields.length === 0) {
        throw inputError('at least one of name, description, enabled, icon must be provided');
    }

    await updateProduct(args.product, args.user, patch);

    const summary = fields.map((f) => (f === 'config' ? 'icon' : f)).join(', ');
    return {
        content: [{ type: 'text', text: `Updated product "${args.product}" (${summary}).` }],
        isError: false,
    };
}
...
async function toolProductPlaybookRun(args) {
    if (!args.product) throw inputError('product is required');
    if (!args.name) throw inputError('name is required');
    if (!args.device_id) throw inputError('device is required (single-device only)');
    ...
    pb.target.devices = [args.device_id];
    ...
    const device = all.find((d) => d.device === args.device_id) || {
        device: args.device_id,
...
async function toolProductPlaybookRollout(args) {
    ...
    const batchSize =
        Number.isFinite(args.batch_size) && args.batch_size > 0 ? Math.floor(args.batch_size) : 5;
...
export const tools = [
    ...
    {
        name: 'thinr_product_create',
        description: `Create a new product. The product ID is immutable — pick it carefully (letters, digits, dot, dash, underscore). Optionally set the display name, description and icon at creation time; otherwise sensible defaults apply (name = id, generic description, Font Awesome Linux icon).

Failure mode: refuses to overwrite an existing product. Use thinr_product_update to change metadata on a product that already exists.

Typical flow: thinr_products to discover what is already there, thinr_product_create to register the new one, thinr_product_script_write to attach scripts, thinr_product_metric_set to register dashboard metrics, thinr_device_set_product to assign devices.`,
        inputSchema: {
            type: 'object',
            properties: {
                product: {
                    type: 'string',
                    description:
                        'Stable product ID (letters, digits, dot, dash, underscore). Cannot be changed later.',
                },
                name: {
                    type: 'string',
                    description:
                        'Display name shown across the platform UI. Defaults to the product ID.',
                },
                description: {
                    type: 'string',
                ...
                icon: {
                    type: 'object',
                    description:
                        "Optional icon. Pick `type: 'fa'` for a Font Awesome class or `type: 'svg'` for inline SVG markup. Defaults to a generic Linux icon.",
                    properties: {
                        type: {
                            type: 'string',
                            enum: ['fa', 'svg'],
                            description:
                                "Icon kind: 'fa' (default) for Font Awesome class, 'svg' for inline SVG markup.",
                        },
                        source: {
                            type: 'string',
                            description:
                                'Font Awesome class (e.g. "fab fa-linux", "fas fa-server") or full SVG markup including the `<svg>` wrapper.',
                        },
                        color: {
                            type: 'string',
                            description: 'Hex foreground color. Default: #000000.',
                        },
                        background: {
                            type: 'string',
        ...
        handler: toolProductCreate,
    },
    {
        name: 'thinr_product_update',
        description: `Update product metadata in place. Pass any subset of fields (\`name\`, \`description\`, \`enabled\`, \`icon\`) — only the supplied ones are modified, the rest are left untouched. Useful to set a friendlier display name shown across the platform UI, change the description, toggle the product on or off, or replace its icon.

Icons accept Font Awesome classes (\`{ type: "fa", source: "fab fa-linux" }\`) or inline SVG markup (\`{ type: "svg", source: "<svg ...>...</svg>" }\`). When an icon is supplied, the current product config is fetched first so unrelated fields (type, profile, dashboards) are preserved.

Does not change the product ID. To rename the ID itself, the product has to be recreated and devices reassigned.`,
        inputSchema: {
            type: 'object',
            properties: {
                product: { type: 'string', description: 'Product ID to update.' },
                name: {
                    type: 'string',
                    description:
                        'Display name shown across the platform UI (dashboards, lists, etc.).',
                },
                description: {
                    type: 'string',
                    description: 'Free-form description.',
                ...
                enabled: {
                    type: 'boolean',
                    description:
                        'Enable or disable the product. Disabled products do not deliver scripts or buckets to devices.',
                },
                icon: {
                    type: 'object',
                    description:
                        "Replace the product's icon. Pick `type: 'fa'` for a Font Awesome class or `type: 'svg'` for inline SVG markup.",
                    properties: {
                        type: {
                            type: 'string',
                            enum: ['fa', 'svg'],
...
Product scripts are executable files kept in a per-product storage ("<product>") under scripts/. Each script file is exposed as a product API resource (same name without the extension) that can be invoked on any device of the product via thinr_resource_call. Compared to device-level scripts (thinr_script_*), product scripts scale to large fleets because the source of truth lives server-side and is downloaded inline on each invocation — no per-device deployment, no drift.`,
    ...
    },
    {
        ...
        description: `Upsert a product-level script: installs a new script if the name is unused, replaces the existing one in place otherwise. The product must exist already — create it first with thinr_product_create. This tool only handles the script side: creates the product's storage if missing, writes the generic index.js wrapper on first use, wires profile.code.storage, enables the product, uploads the script under scripts/<name>, and creates/updates the API resource so the script is immediately callable on any device of the product.

Naming convention: \`*_write\` denotes upsert, shared with every per-product leaf. The product container itself uses split thinr_product_create / thinr_product_update tools to make accidental overwrites of product metadata impossible.
...
Prefer this over iterating \`thinr_exec\` device-by-device when you need to roll out or inspect something across a fleet. Offline devices are skipped by default; pass \`all: true\` to attempt them anyway (they will typically fail fast with a connection error). \`concurrency\` bounds how many devices run at once (default 10), \`timeout\` is per-device in seconds (default 30), and \`fail_fast\` stops dequeueing new devices as soon as one returns a non-zero exit / times out — in-flight work still completes.`,
    ...
    },
    {
        ...
        description: `Upsert a product-level property: creates the property if missing, overwrites it wholesale if it already exists. The value can be any JSON-serialisable shape (string, number, object, array). Prefer the higher-level thinr_product_metric_* tools when managing dashboard metrics — they encapsulate the property shape so you don't have to reconstruct it by hand.

Naming convention: this tool follows the upsert pattern shared by every per-product leaf (\`*_set\`, \`*_write\`). Container-level operations (the product itself) split create and update into separate tools so a fresh registration cannot accidentally overwrite an existing product's metadata.`,
        inputSchema: {
            type: 'object',
            properties: {
                product: { type: 'string', description: 'Product ID.' },
    ...
    },
    {
        ...
        description: `Upsert a dashboard metric on a product: creates a new entry if the \`name\` key is unused, overwrites the existing entry in place otherwise. Under the hood this edits the \`${DASHBOARD_METRICS_PROPERTY}\` product property — prefer this tool over thinr_product_property_set for dashboard metrics so the shape stays consistent.

Typical flow: create the product first (thinr_product_create), write a script that returns the raw data (thinr_product_script_write), then register the metric so the dashboard knows how to surface it.

Naming convention: \`*_set\` denotes upsert, shared with every per-product leaf. The product container itself uses split thinr_product_create / thinr_product_update tools to make accidental overwrites of product metadata impossible.`,
        inputSchema: {
            type: 'object',
            properties: {
                product: { type: 'string', description: 'Product ID.' },
                ...
                aggregation: {
                    type: 'string',
                    description:
                        'How to combine the per-device values across the fleet. Numeric reducers: "sum", "avg", "max", "min". "count" returns the number of devices reporting any non-null value, irrespective of value. "top" lists the values without counting repetitions. "none" returns the raw per-device array. "distribution" returns a flat object mapping each distinct reported value (coerced to string) to the number of devices reporting it — use it for categorical data such as agent version, release/branch, kernel, hostname pattern; it is the right choice when the question is "how is the fleet split across these values?". The output of "distribution" is an object value→count, not a number; the dashboard renders it as a sorted list with per-value percentages and a total row.',
                    enum: ['sum', 'avg', 'max', 'min', 'count', 'top', 'none', 'distribution'],
                },
                visualization: {
                    type: 'string',
                    description:
                        'Dashboard rendering hint. "kpi" (default) shows the aggregated value plus its age. "sparkline" renders the same value alongside a ▁▂▃▅▇ bar of the last ~32 samples, useful for metrics where the trend matters (throughput, connected devices). "list" pairs naturally with aggregation "distribution" — it lays out each value as its own row with count and percentage, plus a total. "bar" is reserved for future use and currently falls back to kpi.',
    ...
    },
    {
        ...
        description: `Upsert a playbook on a product from inline YAML: creates a new playbook if the name is unused, replaces the existing one in place otherwise. This is the upload step of the natural-language authoring flow: after the user describes what they want ("update the nginx package and restart the service on product X"), draft the YAML, iterate against thinr_playbook_validate until it is clean, then call this tool with the validated content.

The YAML is re-validated here against the playbook schema; an invalid document fails fast with a descriptive error and nothing is written — never pass \`skip_validation: true\` to work around validation errors, fix the YAML instead. Uploads are two-phase and self-healing: the file is written under \`playbooks/<name>.yaml\` in the product storage first, then the entry is registered in the \`playbooks\` product property; if the index update fails on a brand-new playbook, the file is rolled back so the index never points at a missing file.

Name rules: letters, digits, underscore, dash — no extension, no slashes. \`description\` is optional; when omitted, the playbook's own top-level \`description\` (if any) is recorded in the index. Author from scratch by calling thinr_playbook_schema first so the generated YAML matches the schema on the first draft.
        ...
        inputSchema: {
            ...
            properties: {
                ...
                skip_validation: {
                    type: 'boolean',
                    description:
                        'Escape hatch for rare cases where the schema cannot yet express a field the runtime already accepts (default: false). Do NOT set this to work around a validation error reported by thinr_playbook_validate — fix the YAML instead. An invalid document will still fail at run time.',
                },
                user: { type: 'string', description: 'API user' },
            },
            required: ['product', 'name', 'content'],
        ...
        inputSchema: {
            ...
            properties: {
                ...
                device_id: {
                    type: 'string',
                    description:
                        'Target device ID (single-device only). Pick any device of the product.',
                },
                vars: {
                    type: 'object',
                    description:
            ...
            required: ['product', 'name', 'device_id'],
        },
