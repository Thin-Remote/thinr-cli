...
async function toolExec(args) {
    ...
    // Default to 30s so a hung command doesn't lock the session
    // indefinitely. Callers can pass a larger value for known-long
    // operations (compiles, backups, …).
...
async function toolMonitoring(args) {
    ...
    if (args.realtime) {
        ...
        if (mon.memory)
            parts.push(
                `Memory: ${mon.memory.usage?.toFixed(1)}% (available: ${(mon.memory.available / 1073741824).toFixed(1)}GB / total: ${(mon.memory.total / 1073741824).toFixed(1)}GB)`,
            );
        if (mon.disk?.root)
            parts.push(
                `Disk: ${mon.disk.root.usage?.toFixed(1)}% (available: ${(mon.disk.root.available / 1073741824).toFixed(1)}GB / total: ${(mon.disk.root.total / 1073741824).toFixed(0)}GB)`,
            );
        if (mon.network)
            parts.push(
    ...
    if (Array.isArray(data) && data.length > 0) {
        output = data
            .map((d) => {
                ...
                if (d.network)
                    parts.push(
                        `net_rx=${(d.network.rx_rate / 1024)?.toFixed(1)}KB/s tx=${(d.network.tx_rate / 1024)?.toFixed(1)}KB/s`,
                    );
