...
export const tools = [
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                device_id: { type: 'string', description: 'Device ID (from thinr_devices)' },
                timeout: { type: 'number', description: 'Timeout in seconds (default: 30)' },
                user: { type: 'string', description: 'API user (for admin impersonation)' },
            },
            required: ['command', 'device_id'],
        },
        handler: toolExec,
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                items: {
                    type: 'number',
                    description:
                        'Number of data points to return (default: 10). In multi-device mode set ≥ number of devices (e.g. 200) since each device contributes one row.',
                },
                minutes: {
                    type: 'number',
                ...
                sort: {
                    type: 'string',
                    description:
                        'Sort order by timestamp: "asc" or "desc" (default: "desc"). Does NOT sort by metric — do that client-side on the result.',
                },
                agg: {
                    type: 'string',
                    description:
                ...
                agg_type: {
                    type: 'string',
                    description:
                        'Reducer applied to each group/window. Supported: "mean", "max", "min", "sum", "count", "first", "last", "median", "spread" (max-min), "stddev". Most common for monitoring: max, mean, min.',
                },
                fields: {
                    type: 'string',
                    description:
