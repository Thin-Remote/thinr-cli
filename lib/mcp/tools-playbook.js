...
export const tools = [
    {
        ...
        description: `Return the JSON schema a ThinR playbook document must follow: accepted top-level fields, the target block, the \`vars\` block (plain and extended forms with \`default\` / \`description\` / \`type\` / \`overridable\` / \`required\`), the variable substitution syntax, and the full catalogue of step actions with their parameters. Call this ONCE at the start of any authoring session — whether the user is asking for a one-shot local playbook (\`thinr_playbook_run\`) or a persistent product playbook (\`thinr_product_playbook_write\`) — so the generated YAML validates on the first try.`,
        inputSchema: {
            type: 'object',
            properties: {},
            required: [],
    ...
    },
    {
        ...
        description: `Parse and validate a playbook document without running it or uploading it. Returns a structured summary (name, description, target, step list) on success, or a consolidated error message listing every issue. Use this while drafting YAML from a natural-language request: write, call validate, fix the reported errors, repeat — then hand the final content to thinr_product_playbook_write (persistent) or thinr_playbook_run (one-shot). Never upload YAML that hasn't validated cleanly; thinr_product_playbook_write will re-validate and reject invalid documents, but running validate during iteration keeps the loop tight without touching product storage.`,
        inputSchema: {
            type: 'object',
            properties: {
                content: {
...
Response carries a consolidated report: devices visited, ok / failed counts, and per-device step outcomes (summary, duration, error message if any). \`isError\` is true whenever any device didn't complete every step cleanly, so clients can branch on a single flag.`,
        inputSchema: {
            ...
            properties: {
                ...
                vars: {
                    type: 'object',
                    description: "Merge into the playbook's `vars` block before substitution.",
                },
                user: {
                    type: 'string',
