// @ts-check
import { getDevices, getDevice, deleteDevice } from '../devices.js';
...
async function toolDevices(args) {
    ...
    // `connection.ts` is when the state last changed, so it reads as
    // "connected since" for a live device and "offline since" for a dead
    // one. Showing it saves a trip to the monitoring history to find out
    // whether something dropped a minute ago or back in July.
    const since = (d) => {
        const ts = d.connection?.ts;
        if (!ts) return '';
        const when = new Date(ts).toISOString().replace('T', ' ').slice(0, 16);
        return d.connection?.active ? `  connected since ${when}` : `  offline since ${when}`;
    };
    ...
    const renderLine = (d) => {
        ...
        return `${status}  ${d.device}${label}${product}${since(d)}`;
    };
...
async function toolDeviceInfo(args) {
    const { api: deviceApi } = getAPI(args.device_id, args.user);
    const parts = [];
    ...
    try {
        const record = await getDevice(args.device_id, args.user);
        const meta = [];
    ...
    try {
        ...
        if (mon.memory)
            parts.push(
                `Memory: ${mon.memory.usage?.toFixed(1)}% (${(mon.memory.total / BYTES.GB).toFixed(1)}GB total)`,
            );
        if (mon.disk?.root)
            parts.push(
                `Disk: ${mon.disk.root.usage?.toFixed(1)}% (${(mon.disk.root.total / BYTES.GB).toFixed(0)}GB total)`,
            );
        if (mon.network)
            parts.push(
...
async function toolDeviceDelete(args) {
    const device = args.device_id;
    if (!device) throw inputError('device is required');
    const removed = await deleteDevice(device, args.user);
    return {
        content: [
            {
                type: 'text',
                text: removed ? `Deleted ${device}` : `Device ${device} did not exist`,
            },
        ],
        isError: false,
    };
...
export const tools = [
    ...
    {
        name: 'thinr_device_delete',
        description:
            'Delete a device record from the platform. Destructive and irreversible — drops the device, its server-side metadata, and stored properties. The agent on the host (if still running) will be left orphaned and will not auto-recreate the record. Idempotent: returns success when the device was already absent.',
        inputSchema: {
            type: 'object',
            properties: {
