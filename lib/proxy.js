...
export async function handleProxyAction(deviceId, protocol, target, options) {
    ...
    if (proxyConfig.web) {
        ...
        spinner.succeed(
            `HTTP proxy running at ${chalk.blue(proxy.url)} → ${deviceId} -> ${chalk.cyan(targetUrl)}`,
        );
        ...
        spinner.succeed(
            `${protocol.toUpperCase()} proxy running on ${chalk.blue(proxy.serverHost + ':' + proxy.serverPort)} → ${deviceId} (${address}:${port})${secureInfo}`,
        );
    }
