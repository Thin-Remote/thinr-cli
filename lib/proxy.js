...
import { PROXY_PORTS } from './constants.js';
import { apiError } from './errors.js';
import { info, accent, hint, warning, error as errorStyle } from './format.js';
...
export async function handleProxyAction(deviceId, protocol, target, options) {
    ...
    if (proxyConfig.web) {
        ...
        spinner.succeed(
            `HTTP proxy running at ${info(proxy.url)} → ${deviceId} -> ${accent(targetUrl)}`,
        );
        ...
        spinner.succeed(
            `${protocol.toUpperCase()} proxy running on ${info(proxy.serverHost + ':' + proxy.serverPort)} → ${deviceId} (${address}:${port})${secureInfo}`,
        );
    }

    console.log(hint(`Press Ctrl+C to stop`));
 ...
 */
function getRandomPort(min = PROXY_PORTS.MIN, max = PROXY_PORTS.MAX) {
...
async function deleteProxy(proxyId) {
    ...
    try {
        await api.delete(`/v1/proxies/${proxyId}`);

        return true;
    } catch (error) {
        console.error(errorStyle(`Error deleting proxy: ${error.message}`));
        return false;
    }
}
...
async function openBrowser(url) {
    try {
        ...
        console.error(errorStyle(`Error opening browser: ${error.message}`));
        return false;
    }
}
