...
/**
 * Format the richer `{name, fn, in?, out?}` entries produced by
 * listDeviceResourcesWithSchemas. Shared between CLI (with chalk) and
 * any other human-facing surface.
 */
export function formatDeviceResourcesWithSchemas(deviceId, entries) {
    let out = `\nResources for device ${chalk.cyan(deviceId)}:\n`;
    if (!entries || entries.length === 0) {
        return out + chalk.yellow('No resources found for this device.\n');
    }
    for (const e of entries) {
        const kind = getResourceType({ fn: e.fn });
        out += `\n- ${chalk.green(e.name)}  ${chalk.gray('[' + kind + ']')}\n`;
