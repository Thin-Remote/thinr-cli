...
export class CommandStream {
    ...
    _buildUrl() {
        ...
        return `wss://${server}${device(apiUser, deviceId)}/resources/${this.resource}?${params}`;
    }
    ...
    _wireEvents(ws) {
        ...
        ws.on('unexpected-response', (req, res) => {
            // Handshake rejected before the upgrade (401 expired token,
            // 404 unknown device, ...). Tag the HTTP status so callers
            // can react to it; execStream() refreshes and retries on 401.
            req.destroy();
