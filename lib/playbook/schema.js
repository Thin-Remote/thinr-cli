...
export const ACTIONS = [
    ...
    {
        name: 'write',
        description: 'Write inline content to a file on the device. Use `push` to upload a local file instead.',
        params: [
            { name: 'path', type: 'string', required: true, description: 'Absolute path on the device.' },
            { name: 'content', type: 'string', required: true, description: 'Inline content to write.' },
        ],
    ...
    },
    {
        ...
        params: [
            { name: 'source', type: 'string', required: true, description: 'Local file path (relative paths resolve against the playbook directory).' },
            { name: 'destination', type: 'string', required: true, description: 'Absolute path on the device.' },
        ],
        summary: (p) => `push ${p.source} → ${p.destination}`,
    },
    {
        name: 'pull',
        description: 'Download a remote file from the device to the local disk.',
        params: [
            { name: 'source', type: 'string', required: true, description: 'Absolute path on the device.' },
            { name: 'destination', type: 'string', required: true, description: 'Local file path (relative paths resolve against the playbook directory).' },
        ],
        summary: (p) => `pull ${p.source} → ${p.destination}`,
    },
    {
        name: 'rm',
        description: 'Remove a file or directory on the device.',
        params: [
            { name: 'path', type: 'string', required: true, description: 'Absolute path on the device.' },
    ...
    },
    {
        ...
        params: [
            { name: 'path', type: 'string', required: true, description: 'Absolute directory path on the device.' },
        ],
        summary: (p) => `mkdir ${p.path}`,
    },
    {
        name: 'mv',
        description: 'Move or rename a path on the device. Handy for atomic deploys (write to tmp, mv into place).',
        params: [
            { name: 'source', type: 'string', required: true, description: 'Path to move from.' },
            { name: 'destination', type: 'string', required: true, description: 'Target path.' },
            { name: 'overwrite', type: 'boolean', description: 'Overwrite destination if it exists (default: false).' },
        ],
        summary: (p) => `mv ${p.source} → ${p.destination}`,
    },
    {
        name: 'property_set',
...
export function playbookSchema() {
    return {
        ...
        document: {
            ...
            vars: 'Mapping of variable name → value (plain form) or name → metadata object (extended form — see `vars_extended`). Referenced in step params with `{{ var }}`.',
            steps: 'Ordered list of steps, executed sequentially on each device.',
        },
        vars_extended: {
            description:
                'Extended form for a single `vars` entry. Use it when a variable needs a description, an explicit type for override validation, or must be marked non-overridable or required.',
            fields: {
                default: 'Default value. Optional when `required: true`. Must match the declared `type`.',
                description: 'Optional human-readable text shown in the dashboard form.',
                type: 'Expected value type for overrides: "string", "number", "boolean", or "object".',
                overridable:
                    'Whether CLI / MCP / dashboard callers may override the value at run time. Defaults to `true`. Set `false` for internal variables.',
                required:
                    'Whether a value is mandatory at run time. Defaults to `false`. If `true` and no default is set, an override must be provided.',
            },
            example: {
                port: { default: 8080, description: 'Service listening port.', type: 'number' },
                internal_token: { default: 'fixed', overridable: false },
                release: { type: 'string', required: true, description: 'Release tag to deploy.' },
            },
        },
        step: {
