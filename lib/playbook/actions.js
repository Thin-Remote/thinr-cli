...
function resolveLocalFile(relPath, baseDir) {
    return readFileSync(resolveLocalPath(relPath, baseDir), 'utf8');
}

const MAX_CAPTURED_STREAM_BYTES = 32 * 1024;

function capStream(s) {
    if (typeof s !== 'string' || s.length <= MAX_CAPTURED_STREAM_BYTES) return s;
    return (
        s.slice(0, MAX_CAPTURED_STREAM_BYTES) +
        `\n…[truncated, ${s.length - MAX_CAPTURED_STREAM_BYTES} more bytes]`
    );
}

/** @type {Record<string, (ctx: any, params: any) => Promise<string | { summary: string, stdout?: string, stderr?: string, exitCode?: number }>>} */
export const HANDLERS = {
    async exec(ctx, params) {
        ...
        const { exitCode, timedOut } = await ctx.api.execStream(params.command, {
            timeout,
            onStdout: (s) => {
                if (stdout.length < MAX_CAPTURED_STREAM_BYTES) stdout += s;
            },
            onStderr: (s) => {
                if (stderr.length < MAX_CAPTURED_STREAM_BYTES) stderr += s;
            },
        });
        const capturedStdout = capStream(stdout);
        const capturedStderr = capStream(stderr);
        if (timedOut) {
            const err = new Error(`timed out after ${timeout}s`);
            // @ts-ignore — attach captured streams for the runner to surface
            err.stdout = capturedStdout;
            // @ts-ignore
            err.stderr = capturedStderr;
            throw err;
        }
        if (exitCode !== 0) {
            const err = new Error(`exit=${exitCode}`);
            // @ts-ignore
            err.stdout = capturedStdout;
            // @ts-ignore
            err.stderr = capturedStderr;
    ...
    async property_set(ctx, params) {
        await setDeviceProperty(ctx.deviceId, params.property, params.value, ctx.user);
        return `set ${params.property}`;
    },

    async resource(ctx, params) {
        const result = await callDeviceResource(
            ctx.deviceId,
            params.resource,
            params.inputs || null,
            {
        ...
        const summary =
            typeof result === 'string' ? result.slice(0, 40) : JSON.stringify(result).slice(0, 40);
    ...
    async script_install(ctx, params) {
        ...
        const info = await readDeviceResource(ctx.deviceId, '$scripts/info', { user: ctx.user });
        ...
        await callDeviceResource(ctx.deviceId, '$scripts/reload', {}, { user: ctx.user });
...
export const CHECKERS = {
    ...
    async pull(ctx, params) {
        try {
            ...
            return {
                status: 'changed',
                summary: `would pull ${params.source} (${size} bytes) → ${params.destination}`,
            };
        } catch (err) {
            if (err?.response?.status === 404) {
                return {
                    status: 'changed',
                    summary: `source ${params.source} not found on device`,
                };
            }
            throw err;
        }
    ...
    async property_set(ctx, params) {
        let current;
        try {
            current = await getDeviceProperty(ctx.deviceId, params.property, ctx.user);
        } catch (err) {
    ...
    async update(ctx, params) {
        ...
        return {
            ...
            summary:
                `update ${params.op}: ${result?.status || 'unknown'}` +
...
async function hashCheck(api, path, contentBuffer) {
    ...
    try {
        ...
        if (err?.response?.status === 400) {
            return {
                status: 'changed',
                summary: `${path} is not a regular file — would overwrite`,
            };
        }
        if (err?.response?.status === 405 || err?.response?.status === 501) {
            return { status: 'unknown', summary: 'agent lacks $fs/hash — cannot predict' };
