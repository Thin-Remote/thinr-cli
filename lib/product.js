...
export async function createProduct(
    ...
    // Icons accept either a Font Awesome class (string, legacy short form)
    // or a structured object { type: 'fa'|'svg', source, color, background }.
    // The structured form lets callers ship inline SVG markup directly.
    const iconSpec =
        typeof icon === 'object' && icon !== null
            ? {
                  type: icon.type === 'svg' ? 'svg' : 'fa',
                  source: icon.source,
                  color: icon.color || '#000000',
                  background: icon.background || '#ffffff',
              }
            : { type: 'fa', source: icon, color: '#000000', background: '#ffffff' };
    try {
 ...
 */
export async function installProductScript({ product, name, content, user, input, output } = {}) {
    ...
    // 1. Refuse to silently create the product. The agent flow is
    //    list → create → write-script: if the product is missing, the
    //    caller has to create it explicitly with `thinr_product_create`
    //    so its name, description and icon are set deliberately rather
    //    than picking up defaults from a side effect.
    if (!(await productExists(product, user))) {
        const err = new Error(
            `Product "${product}" does not exist. Create it first with thinr_product_create.`,
        );
        err.code = 'not_found';
        throw err;
    }
...
/**
 * Persist a fleet-run report in the product's file storage under
 * `playbooks/runs/`. Returns `{ ok, path, reason }`. Best-effort: the
 * caller decides whether to surface failures as warnings instead of
 * aborting, since the rollout has already run by the time this is
 * called.
 *
 * @param {{
 *   product: string,
 ...
 * }} opts
 */
export async function uploadFleetRunReport({ product, report, user, filename } = {}) {
    if (!product) throw inputError('product is required');
    if (!report || typeof report !== 'object') throw inputError('report is required');

    if (!(await storageExists(product, user))) {
        return { ok: false, reason: 'storage-missing', path: null };
    }
    const path = `playbooks/runs/${filename || defaultRunReportFilename(report)}`;
    try {
        await writeStorageFile(product, path, JSON.stringify(report, null, 2), user);
...
function defaultRunReportFilename(report) {
    const name = typeof report.name === 'string' && PLAYBOOK_NAME_RE.test(report.name)
        ? report.name
        : 'run';
    const user = typeof report.user === 'string' && /^[A-Za-z0-9_.-]+$/.test(report.user)
        ? report.user
        : 'unknown';
    const ts = typeof report.startedAt === 'string' ? report.startedAt : new Date().toISOString();
    const stamp = ts.replace(/[:.]/g, '').replace(/[^0-9TZ]/g, '');
    return `${stamp}-${name}-${user}.json`;
}
