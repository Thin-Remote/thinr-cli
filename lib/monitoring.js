...
export async function getMonitoringData({
    ...
    if (fields) {
        ...
        const hasDevice = fields
            ...
            .map((s) => s.trim())
            .includes('device');
        params.fields = !device && !effectiveGroupBy && !hasDevice ? `device,${fields}` : fields;
    }

    const url = device
        ? `${v3(user)}/devices/${device}/buckets/monitoring/data`
        : `${v2(user)}/buckets/monitoring/data`;
