...
export async function getMonitoringData({
    ...
    if (fields) {
        // Multi-device queries need `device` to attribute each sample; if
        // the caller filtered it out, rows come back anonymous.
        const hasDevice = fields
            .split(',')
            .map((s) => s.trim())
            .includes('device');
        params.fields = !device && !hasDevice ? `device,${fields}` : fields;
    }
